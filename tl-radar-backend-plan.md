...
> **Исторический документ.** Это исходное техническое задание, написанное до
> начала разработки (Этап 0). Часть описанных здесь решений впоследствии
> изменилась по мере развития проекта — например, вход стал по имени+фамилии
> без email (см. `lib/auth/synthetic-email.ts`), роль `viewer` перестала
> назначаться при регистрации (`0006_remove_viewer_role.sql`), а одиночный
> `tasks.person`/`tasks.assignee_id` был заменён таблицей-связкой
> `task_assignees` с поддержкой нескольких исполнителей
> (`0010_task_assignees.sql`). Файл сохранён как есть для истории замысла;
> **актуальное описание системы — [`README.md`](./README.md)**.
