...
- `supabase/migrations/0010_task_assignees.sql` — `tasks.assignee_id`
  заменён таблицей-связкой `task_assignees` (`assignee_id`, `added_at`,
  `added_by`, `removed_at`, `removed_by`) — у задачи теперь может быть
  несколько исполнителей. Удаление исполнителя — мягкое (`removed_at`),
  не физическое. Существующие одиночные `assignee_id` перенесены в
  `task_assignees` с `added_at = tasks.created_at`. Важное правило (важно
  для будущей аналитики, Этап 6): время засчитывается исполнителю только
  с момента его фактического добавления (`added_at`), не задним числом —
...
- `supabase/migrations/0011_task_estimate.sql` — `tasks.estimated_minutes`
  (необязательно, `check (> 0)`). Используется только для индикатора
  «оценка vs факт» на карточке задачи и в архиве; переиспользует
  существующий `accumulatedInProgressDuration()` — новой бизнес-логики
  учёта времени не вводит. Полоса прогресса меняет цвет на «перерасход»
  (`--brand`), если фактическое время превышает оценку.
...
- `lib/logic/tasks.ts` — `accumulatedInProgressDuration()` теперь построена
  поверх экспортируемой `inProgressIntervals()` (список интервалов времени
  в статусе in_progress, за вычетом закрытых пауз). Поведение
  `accumulatedInProgressDuration()` не изменилось — проверено рандомным
  тестом на 20 000 реалистичных сценариев (пауза не может начаться раньше
  `started_at` и не может пересекать `completed_at` — это гарантируется
  существующей бизнес-логикой переходов статусов).
- `lib/logic/analytics.ts` — `availableWorkingMs()` (доступное рабочее
  время 07:30–16:00 по Мюнхену за период) и `computePersonStats()`
  (время каждого человека «на задачах» за период — пересечение
  `inProgressIntervals()` задачи с окном `[added_at, removed_at]` каждого
  назначения из `task_assignees`, включая уже снятые назначения).
- `lib/logic/access.ts` — `STATS_MIN_ROLE` (сейчас `'editor'`, то есть
  видно всем зарегистрированным) — единая именованная константа; решение
  об ограничении аналитики только ролью `admin` откладывается на потом и
  не требует правок где-либо ещё, кроме этой константы.
...
- Новой миграции для этого этапа нет — используются существующие таблицы
  `tasks`, `task_pauses`, `task_assignees`, `profiles`.
